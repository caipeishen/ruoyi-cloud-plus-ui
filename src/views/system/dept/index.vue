  ...
  <div class="p-2">
    ...
    <el-card shadow="hover">
      <template #header>
        <el-row :gutter="10">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="Plus" @click="handleAdd()" v-hasPermi="['system:dept:add']" v-if="checkRole(['superadmin','admin'])">新增 </el-button>
          </el-col>
          <el-col :span="1.5">
      ...
      >
        <el-table-column prop="deptName" label="部门名称" width="400">
          <template #default="scope">
            <svg-icon :icon-class="DeptTypeEnum[scope.row.deptType]" />&nbsp;
            {{scope.row.deptName}}
          </template>
        </el-table-column>
    ...
    <el-dialog :title="dialog.title" v-model="dialog.visible" destroy-on-close append-to-bod width="600px">
      <el-form ref="deptFormRef" :model="form" :rules="checkRole(['superadmin','admin']) ? rulesAdmin : rules" label-width="80px">
        <el-row>
          <el-col :span="12" v-if="form.parentId!=undefined">
...
const { queryParams, form, rules, rulesAdmin } = toRefs<PageData<DeptForm, DeptQuery>>(data)

/** 默认打开当前部门 */
const getDefaultExpandKeys = async () => {
  if (userStore && userStore.deptRoute) {
    let deptIdList = userStore.deptRoute.split(',');
    expandKeys.value = deptIdList;
  }
}
