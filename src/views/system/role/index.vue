      ...
      <el-form :model="form" label-width="80px" ref="dataScopeRef">
        ...
        <el-form-item label="权限范围">
          <el-select v-model="form.dataScope" @change="dataScopeSelectChange">
            <el-option v-for="item in sys_data_scope" :key="item.value" :label="item.label" :value="item.value"></el-option>
          </el-select>
        </el-form-item>
          ...
          >
            <template #default="{ node, data }">
              <span class="custom-tree-node">
                <span>
                  <svg-icon :icon-class="DeptTypeEnum[data.deptType]" />&nbsp;
                  {{ node.label }}
                </span>
              </span>
            </template>
...
const { sys_normal_disable, sys_data_scope } = toRefs<any>(proxy?.useDict('sys_normal_disable', 'sys_data_scope'));
